  ...
  if (securityName === 'jwt') {
    ...
    if (decodedPayload) {
       // : Check role directly from token payload
        ...
        const user = await userRepository.findOne({where:{ id: decodedPayload.user.id }, relations: ["role","status"] });
        if (!user) {
        ...
        if(user.role.id !== userRole.id){
          throw new HttpError(403, "User role has changed, please re-authenticate");
        }
      ...
      console.log("Checking scopes:", scopes);
        if (scopes && scopes.length > 0) {
          ...
          for (let scope of scopes) {
            if (scope.includes(decodedPayload.user.role.name)) {
              hasPermission = true;
              break;
            }
          }
